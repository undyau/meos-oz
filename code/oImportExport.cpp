...
bool oEvent::exportOECSV(const wchar_t *file, int languageTypeIndex, bool includeSplits, bool byClass)
{
  enum {
  ...
  {
  case 1: // English
    csv.outputRow("Stno;SI card;Database Id;Surname;First name;YB;S;Block;nc;Start;Finish;Time;Classifier;Club no.;Cl.name;City;Nat;Cl. no.;Short;Long;Num1;Num2;Num3;Text1;Text2;Text3;Adr. name;Street;Line2;Zip;City;Phone;Fax;EMail;Id/Club;Rented;Start fee;Paid;Course no.;Course;km;m;Course controls;Pl"); 
    maleString = "M";
    femaleString = "F";
    break;
  case 2: // Svenska
  ...
  for (it = Runners.begin(); it != Runners.end(); ++it) {
    ...
    if (it->getFinishTime() > 0)
      row[OEfinish] = gdibase.recodeToNarrow(it->getFinishTimeS());
...
bool oEvent::addOECSVCompetitorDB(const vector<wstring> &row)
{
  ...
  int birth = _wtoi(row[OEbirth].c_str());
...
void oEvent::importXML_IOF_Data(const wstring &clubfile,
                              const wstring &competitorfile, bool clear)
{
  if (!clubfile.empty()) {
    xmlparser xml_club;
...
bool oEvent::addXMLClass(const xmlobject &xclass)
{
  ...
  for (size_t k = 0; k<xFee.size(); k++) {
    ...
    if (xamount) {
      double f = atof(xamount.getRaw());
      wstring cur = xamount.getAttrib("currency").wget();
      if (f>0) {
          ...
          xml.startTag("Result");
            ...
            if (pcourse && legStatus>0 && legStatus!=StatusDNS && legStatus!=StatusCANCEL) {
              ...
              for (int k=startIx;k<endIx;k++) {
                ...
                if (unsigned(k)<sp.size() && sp[k].time>0)
                  xml.write("Time", "clockFormat", L"HH:MM:SS", getAbsTime((sp[k].time-it->tStartTime)-ZeroTime, true));
                else
                  xml.write("Time", L"--:--:--");
      ...
      xml.startTag("Result");
        ...
        if (pcourse && it->getStatus()>0 && it->getStatus()!=StatusDNS
          ...
          for (int k=startIx;k<endIx;k++) {
            ...
            xml.write("ControlCode", pcourse->Controls[k]->getFirstNumber());
            if (unsigned(k)<sp.size() && sp[k].time>0)
              xml.write("Time", "timeFormat", L"HH:MM:SS", getAbsTime((sp[k].time-it->tStartTime)-ZeroTime, true));
            else
              xml.write("Time", L"--:--:--");
