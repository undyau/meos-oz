...
{
  eventorBase = poe->getPropertyString("EventorBase", L"https://eventor.orienteering.asn.au/api/");
...
int TabCompetition::competitionCB(gdioutput &gdi, int type, void *data)
{
  if (type == GUI_LINK) {
    ...
    if (bi.id == "CopyLink") {
      ...
      eventorCand.push_back(L"https://eventor.orienteering.asn.au/api/");
...
void TabCompetition::textSizeControl(gdioutput &gdi) const
{
  ...
  wstring current = oe->getPropertyString("Language", L"English");
  int ix = -1;
